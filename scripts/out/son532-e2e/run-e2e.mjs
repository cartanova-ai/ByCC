  ...
  for (const mode of ["generate", "stream"]) {
    ...
    console.log(
      ...
      r.transportError
        ...
        : `syntax=${record.classification.syntax} missing=${record.classification.missingRequired.length} fence=${record.classification.fenceResidue} wall=${r.wall}ms out=${r.usage?.output_tokens} cost=$${r.costUsd?.toFixed?.(4)}`,
    );
  }
}
