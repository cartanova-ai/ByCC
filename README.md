...
- **OpenAI** — Calls `https://chatgpt.com/backend-api/codex/responses` directly. `QGRID_OPENAI_TRANSPORT=https` (the default) uses HTTPS/SSE; `QGRID_OPENAI_TRANSPORT=websocket` scheme-swaps that URL to `wss` and opens one Responses WebSocket per request. Each WebSocket sends one `response.create` message and must receive a terminal Responses event. Qgrid does not multiplex connections or replay ambiguous requests. Only a definitively rejected 401 handshake may refresh credentials and reconnect once. Each token has concurrent permits; new work uses smooth weighted routing across eligible tokens. Invalid selector values fail during dispatcher configuration.
