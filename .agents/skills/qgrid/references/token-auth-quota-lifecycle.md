...
- A periodic digest (`expired-token-digest.ts`) re-announces tokens that are still inactive, because the one-shot death alert is lost on anyone not watching the channel at that moment and re-login only happens when a person does it. It runs every `SLACK_EXPIRY_DIGEST_INTERVAL_MINUTES` (unset or 0 disables) and sends nothing when no token is inactive — a periodic "all clear" trains people to ignore the channel. Owners come from `SLACK_USER_MAP` (`tokenName:SlackUserId`), which is explicit rather than resolved through `users.list`: Slack handles diverge from token names often enough (`haze`→`haze.lee`, `byeongjun`→`potados`) that auto-matching drops the mention silently, and a digest without the mention has no one to act on it. Shared accounts are left out of the map on purpose and appear by name only. Each instance sends its own digest — unlike deactivation there is no state change to gate on, and a few duplicates a day cost less than a locking scheme.
